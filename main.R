...
library(pcaReduce)
library(SOUP)
setwd("[the file path of 'SOUP_ori.R]") # the file pathway of 'SOUP_ori.R'
source("SOUP_ori.R") 
...
name_2_species<-function(scName){
  if(scName %in% c('yan','pollen','patel'))
  {return('human')}
  else{return('mouse')} # c('biase','goolam','deng-rpkms','kolodziejczyk','treutlein','usoskin','klein','zeisel')
}
...
# adding pathway: SNF
adding_pathway <- function(mat_gene, mat_path){
...
clustering_method<- function(mat_gene,mat_path,W,cName,k){
  ...
  switch(which(m_list == cName),
         ...
         labels_prdic= as.numeric(sData@clusters)},
         
         # 8. pcaReduce
         {Output_S <- PCAreduce(W, nbt=1, q=k-1, method='S')
         labels_prdic = Output_S[[1]][,1]},
         
         # 9. SOUP
         {soup.out = SOUP_2(W, Ks=k, type="log")
         soup.labels = soup.out$major.labels[[1]]
         labels_prdic = as.numeric(soup.labels)},
  )
  clust_results = paste(as.character(labels_prdic),collapse=',')
...
#     'yan.rds', 'pollen.rds', 'biase.rds', 'goolam.rds', 'deng-rpkms.rds',
#     'kolodziejczyk.rds', 'usoskin.rds','klein.rds', 'zeisel.rds','camp1.rds'
cName = 'DBSCAN'
paName = 'KEGG'
scName= 'yan'
...
main<-function(cName, paName, scName,s,
  ...
  # adding pathway 
  W=adding_pathway(mat_gene, mat_path)
  
  # clustering
  k = length(unique(label_int)) # real k
  clust_results = clustering_method(mat_gene,mat_path,W,cName,k)
  print(clust_results)
}
